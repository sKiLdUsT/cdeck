function homePage() {
    ...
    setInterval(function () {
        $('.modal').each(function () {
            if (!$(this).is(':visible') && $(this).attr('id') !== 'highlights') {
    ...
    $('#highlights .modal-content h4:first').text('cDeck Version '+$app.version+' Highlights');
    if($('meta[name=highlights]').attr('content') == "true"){
        setTimeout(showHighlights, 2000);
        setTimeout(function(){
            var donate = "$('form#donate_form').trigger('submit')";
            Materialize.toast('cDeck will always be free. But that\'s expensive. If you can spare the coin and want to help us out, consider donating! <button class="btn waves-effect waves-light blue" onclick="'+donate+'"><img class="left" src="/assets/img/pp.png">Donate</button>', 8000);
            setTimeout(function(){
                Materialize.toast('Hint: You can find the donation button in the "About" section of the settings.', 10000);
            }, 5000)
        }, 30000);
    }
}
...
function showHighlights(){
    $('#highlights').css({width: '85%', height: '80%', maxHeight: '80%'}).openModal({
        dismissible: true,
        opacity: .5,
        ready: function () {
